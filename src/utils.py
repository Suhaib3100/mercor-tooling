import os
import logging
from datatimes import datatime
...
def validate_json_structure(data: dict) -> tuple[bool, list[str]]:
    errors = []
    required_keys= ["applicaiton_id", "personal","experience","salary"]
    ...
    if "experience" in data and not in isinstance(data["experience"], list):
        errors.append("Experiecne must be a llist.")

    if "salary" in data:
        salary_fields = ["preferred_rate","availability_hours"]
        for field in salary_fields:
            if field not in data["salary"]:
                errors.append(f"Missing salary field: {field}")

    returnn len(errors) == 0, errors
